...
function setupAuthForms() {
  ...
  const setMessage = (el, message, success = false) => {
    if (!el) return;
    el.textContent = message;
    el.classList.toggle('success', success);
  };

  const clearInputErrors = (inputs) => {
  ...
  if (signupForm) {
    signupForm.addEventListener('submit', (e) => {
      ...
      if (!usernameValue) {
        markError(username);
        setMessage(message, 'Username is required.');
        return;
      }

      if (usernameValue.length > 12) {
        markError(username);
        setMessage(message, 'Username must be 12 characters or less.');
        return;
      }

      if (!emailValue) {
        markError(email);
        setMessage(message, 'Email address is required.');
        return;
      }

      if (!/^[^\s@]+@gmail\.com$/i.test(emailValue)) {
        markError(email);
        setMessage(message, 'Email must be a valid @gmail.com address.');
        return;
      }

      if (!passwordValue) {
        markError(password);
        setMessage(message, 'Password is required.');
        return;
      }

      if (passwordValue.length < 6) {
        markError(password);
        setMessage(message, 'Password must be at least 6 characters.');
        return;
      }

      if (!confirmValue) {
        markError(confirmPassword);
        setMessage(message, 'Please confirm your password.');
        return;
      }
      ...
      setMessage(message, 'Registration successful! Redirecting to login page in 3 seconds...', true);
      setTimeout(() => {
  ...
  if (loginForm) {
    loginForm.addEventListener('submit', (e) => {
      ...
      if (!/^[^\s@]+@gmail\.com$/i.test(emailValue)) {
        markError(email);
        setMessage(message, 'Please enter a valid @gmail.com email address.');
        return;
      }
      ...
      setMessage(message, 'Login validation passed. Firebase login can be connected next.', true);
    });
  }
}
